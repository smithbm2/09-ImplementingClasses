...
class Point(object):
    ...
    def move_to(self, x, y):
        ...
        self.distance1 = self.distance1 + math.sqrt((startx-x)**2 + (starty-y)**2)
    def move_by(self, dx, dy):
        self.x = self.x + dx
        self.y = self.y + dy
        self.totalmoves = self.totalmoves + 1
        self.distance1 = self.distance1 + math.sqrt(((dx)**2 + (dy)**2))
    def get_number_of_moves_made(self):
        return self.totalmoves
    def get_distance_from(self , Point):
        distancex = abs(self.x - Point.x)
        distancey = abs(self.y - Point.y)
        distance = math.sqrt(distancex**2+distancey**2)
        return distance
    def get_distance_from_start(self):
        distancex = abs(self.x - self.startx)
        distancey = abs(self.y - self.starty)
        distance = math.sqrt(distancex**2 + distancey**2)
        return distance
